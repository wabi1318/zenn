...
# 1. コンポーネント・アーキテクチャ
Reactは**コンポーネント**という独立した再利用可能なパーツを組み合わせることでアプリケーションを作るコンポーネントベース・アーキテクチャを採用している。これによりコンポーネントの再利用が可能となる。また、コンポーネントの変更や編集をする際も、一つの変更に合わせて全てのコードを調整する必要がないためバグの修正や保守が楽になる。
...
- Reactで開発する際に用いることがかなり多い言語である
- React + Typescript の採用例としては
  - Slack
  - Airbnb
  - スタディサプリ(リクルート)
  - LINE NEWS
